...
transferred to the visible fixtures. The raw report's `representative_maps`
selection instead retains the exact 95,618-byte input used for the captured
observations; deterministic selection of the live corpus is tested separately.
A new complete measurement run is required before replacing the captured map
metadata, expansion, or timing observations. Follow-up #210 tracks the
remaining historical-provenance regression coverage.
